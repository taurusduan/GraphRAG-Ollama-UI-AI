...
# 设置常量和配置
INPUT_DIR = api_config.get('INPUT_RAG_DIR')
...
async def setup_llm_and_embedder():
    ...
    # 获取API密钥和基础URL
    api_key = api_config.get("GRAPHRAG_API_KEY", "YOUR_API_KEY")
    api_key_embedding = api_config.get("GRAPHRAG_API_KEY_EMBEDDING", api_key)
    api_base = api_config.get("API_BASE", "https://api.openai.com/v1")
    api_base_embedding = api_config.get("API_BASE_EMBEDDING", "https://api.openai.com/v1")

    # 获取模型名称
    llm_model = api_config.get("GRAPHRAG_LLM_MODEL", "gpt-3.5-turbo-0125")
    embedding_model = api_config.get("GRAPHRAG_EMBEDDING_MODEL", "text-embedding-3-small")
